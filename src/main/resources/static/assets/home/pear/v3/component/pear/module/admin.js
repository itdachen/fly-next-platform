layui.define(['message', 'table', 'jquery', 'element', 'yaml', 'form', 'tab', 'menu', 'frame', 'theme', 'convert','fullscreen'],
	function(exports) {
		...
		function applyConfig(param) {
			...
			/* LOGO 加载 */
			config.logo.title = clientName;
			config.logo.image = HTTP_BIZ_URI + $('#logoImage').val();

			/* 动态加载首页页面 */
			config.tab.index.id = $('#mainHomeId').val();
			config.tab.index.href = $('#mainHomeUri').val();
			config.tab.index.title = $('#mainHomeTitle').val();

			/* 菜单加载路径 */
			config.menu.data = $('#permsAuthUri').val();
			config.menu.select = config.tab.index.id;
