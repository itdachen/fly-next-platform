...
public class AdminDashboardController {
    ...
    public String index(ModelMap modelMap) {
        clientInfo(modelMap);
        if (BackstageTemplateEnum.LYEAR == clientProperties.getTemplate()) {
            return LYEAR_PREFIX + "/index";
        } else if (BackstageTemplateEnum.LAYUI == clientProperties.getTemplate()) {
            String permsAuthUri = contextPath() + "/perms/auth/menu";
            modelMap.put("permsAuthUri", permsAuthUri);
            return PATH_PREFIX + "/index";
        } else if (BackstageTemplateEnum.OKADMIN == clientProperties.getTemplate()) {
    ...
    private void clientInfo(ModelMap modelMap) {
        ...
        /* 默认跳转页面 */
        PermissionInfo homeUri = permsAuthService.getHomeUri(clientProperties.getId());
