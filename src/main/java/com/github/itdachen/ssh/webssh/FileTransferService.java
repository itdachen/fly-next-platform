...
public class FileTransferService {
     ...
     * 上传文件到远程服务器
     */
    public void uploadFile(SSHServerInfo server, MultipartFile file, String remotePath) throws Exception {
        ...
        try {
            ...
            logger.info("文件上传成功: {} -> {}", file.getOriginalFilename(), remoteFilePath);
     ...
     * 从远程服务器下载文件
     */
    public byte[] downloadFile(SSHServerInfo server, String remoteFilePath) throws Exception {
        Session session = null;
        ChannelSftp sftpChannel = null;
    ...
    public List<FileInfo> listDirectory(SSHServerInfo server, String remotePath) throws Exception {
        Session session = null;
        ChannelSftp sftpChannel = null;
        ...
        try {
            ...
            logger.info("目录列表获取成功: {}, 文件数: {}", remotePath, files.size());
            return files;
     ...
     * 创建远程目录
     */
    public void createRemoteDirectory(SSHServerInfo server, String remotePath) throws Exception {
        Session session = null;
        ChannelSftp sftpChannel = null;
        ...
        try {
            ...
            createRemoteDirectory(sftpChannel, remotePath);
            logger.info("远程目录创建成功: {}", remotePath);
     ...
     * 删除远程文件或目录
     */
    public void deleteRemoteFile(SSHServerInfo server, String remotePath, boolean isDirectory) throws Exception {
        Session session = null;
        ChannelSftp sftpChannel = null;
        ...
        try {
            ...
            logger.info("远程文件删除成功: {}", remotePath);
     ...
     * 重命名远程文件
     */
    public void renameRemoteFile(SSHServerInfo server, String oldPath, String newPath) throws Exception {
        Session session = null;
        ChannelSftp sftpChannel = null;
     ...
     * 批量上传文件
     */
    public void uploadFiles(SSHServerInfo server, MultipartFile[] files, String remotePath) throws Exception {
        Session session = null;
        ChannelSftp sftpChannel = null;
        ...
        try {
            ...
            logger.info("批量上传完成，共上传 {} 个文件", files.length);
    ...
    private Session createSession(SSHServerInfo server) throws JSchException {
        JSch jsch = new JSch();
        Session session = jsch.getSession(server.getHostUsername(), server.getHostIp(), Integer.parseInt(server.getHostPort()));
        session.setPassword(server.getHostPassword());
